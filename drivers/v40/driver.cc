// pikawire reader driver for pikiwidb 4.0.x dumps (three numeric engine
// instances; Storage facade API shared with 3.5.x for the calls used here).
...
SnapshotIter* snapshot_open(const char* dump_path, const char* db_name, SnapshotOptions opts, char** err) {
  ...
  // Layout guard (4.0.x): Storage::Open LOG(FATAL)s on unexpected engine
  // directories, so pre-validate the three numeric kv/svc/meta instances
  // (<root>/0..2/CURRENT), bootstrap empty skeletons, and refuse a 3.5.x
  // per-type dump with a clean "wrong driver" error instead of aborting.
  ...
  {
    // A 4.0 checkpoint always materializes engines 0/1/2; if none exist but
    // per-type dirs do, this is a 3.5.x dump — wrong driver, clean error.
    if (!std::filesystem::exists(AppendPath(db_root, "0")) &&
        std::filesystem::exists(AppendPath(db_root, "strings"))) {
      set_error(err, "dump layout is pika 3.5.x (per-type dirs) but this driver is v40: loader picked the wrong driver");
      delete it;
      return nullptr;
    }
    static const char* kEngines[] = {"0", "1", "2"};
    bool saw_kv = false;
    for (const char* t : kEngines) {
...
extern "C" const char* pikawire_driver_layout(void) { return "v40"; }
